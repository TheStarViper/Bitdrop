...
void DrawMap() {
    ...
    for (int c = 0; c < Config::totalmapcolumns; c++) {
        for (int r = 0; r < state.columnNodeCounts[c]; r++) {
            ...
            float pixelScale = iconSize / 16.0f;
            Vector2 iconTopLeft = { n->position.x - iconSize / 2.0f, n->position.y - iconSize / 2.0f };

            for (int iy = 0; iy < 16; iy++) {
                for (int ix = 0; ix < 16; ix++) {
                    if (iconGrid[iy][ix]) {
                        DrawRectangle(
                            (int)roundf(iconTopLeft.x + ix * pixelScale),
    ...
    if (state.selectedNode && GetWorldToScreen2D(state.selectedNode->position, state.camera).x < 800&&!esc_menu) {
